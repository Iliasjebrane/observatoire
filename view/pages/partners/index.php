...
<div class="mt-2">
    <div class='d-block'>
        ...
        <div class="d-flex flex-end gap-1">
            <div class="me-auto"><span class=" text text-success">partners</span></div>

            <div class="me-2">
                <a class="btn btn-success btn-sm " href="<?= route('admin/partners/create') ?>"><i
                        ...
                        <tr>
                            ...
                            <td>
                                <i role="button" class="bi bi-eye-fill text-success fs-5"
                                    onclick="showPieceJointe('<?= storage($partner['image']) ?>')"></i>
                            </td>
                            <td>
                                <div class='text-center d-flex gap-1 justify-content-center'>
                                    <form onsubmit="return confirm('Voulez-vous vraiment supprimer ce partner ?')"
                                        action="<?= route("admin/partners/{$partner['id']}") ?>" method="post">
                                        <input type="hidden" name="_method" value="delete">
                                        <button class="btn btn-danger btn-sm bg-gradient"><i
                                                class="bi bi-trash"></i></button>
                                    </form>
                                    <a href="<?= route("/admin/partners/{$partner['id']}/edit") ?>">
                                        <button class="btn btn-success btn-sm bg-gradient">
                                            <i class="bi bi-pencil-square"></i>
                                        </button>
                                    </a>
                                    <a href="<?= route("admin/partners/{$partner['id']}") ?>">
